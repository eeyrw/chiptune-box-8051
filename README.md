...
模拟器：PC 端把 XM 的行、tick、效果和包络展开为紧凑的 `T10M` 控制流，
MCU 端只负责定时应用声道状态和实时波表混音。
...
- 10 个固定逻辑声道，不做动态复音分配。
- 每声道 24-bit DDS 相位、24-bit 步进、5-bit 音量、8 种 16-step 波表。
- Timer0 ISR 中的 10 路热路径由汇编完全展开。
- XM 的 arpeggio、portamento、vibrato、volume slide 在 PC 端逐 tick 展开。
- `T10M` 使用音高查表、小音高差分和 wait 差分压缩；ISR 不参与解码。
...
架构和格式细节见 [docs/Tracker10Architecture.md](docs/Tracker10Architecture.md)，
