...
`T10M v2` 曲谱，MCU 主线程运行受限 tracker VM，32 kHz ISR 只负责合成。
...
- 主线程按 tracker tick 执行 arpeggio、portamento、vibrato、volume slide 和乐器宏。
- `T10M v2` 复用 pattern，并以行单元格字段掩码和固定乐器宏保存高层语义。
